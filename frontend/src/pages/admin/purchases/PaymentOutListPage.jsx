...
export default function PaymentOutListPage() {
  ...
  const filters = { type: 'PAYMENT_OUT', page, perPage: 20 };
  ...
  return (
    <PageShell
      title="Payment Out"
      subtitle={`${total} payment(s) made`}
      actions={
        <Button asChild size="sm">
          <Link to="/admin/purchases/payment-out/new">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={8} className="px-4 py-12 text-center text-[var(--color-text-tertiary)]">No payment out records yet</td></tr>
            ) : (
              rows.map((row) => (
                <tr
                  key={row.id}
                  onClick={() => navigate(`/admin/purchases/payment-out/${row.id}`)}
