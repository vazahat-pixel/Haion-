...
export default function PaymentOutDetailPage() {
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Payment cancelled and purchase settlements reversed');
  ...
  return (
    <DetailPageShell
      ...
      actions={isActive ? (
        <Button
          ...
          onClick={() => {
            if (confirm('Cancel this payment? Purchase settlements will be reversed.')) {
    ...
    >
      <div className="grid gap-6 max-w-3xl">
        <Card>
          <CardHeader><CardTitle className="text-sm font-semibold">Payment Out Summary</CardTitle></CardHeader>
          <CardContent>
            <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm sm:grid-cols-3">
              ...
              <div>
                <dt className="text-xs text-[var(--color-text-secondary)]">Amount Paid</dt>
                <dd className="text-base font-bold text-blue-600 dark:text-blue-400">{formatCurrency(data.amount)}</dd>
              </div>
              <div>
        ...
        {(data.settledPurchases?.length > 0) && (
          <Card>
            <CardHeader><CardTitle className="text-sm font-semibold">Settled Purchases</CardTitle></CardHeader>
            <CardContent>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b bg-surface-2 text-left text-xs uppercase text-[var(--color-text-secondary)]">
                      ...
                      <th className="px-3 py-2 text-right">Purchase Amt</th>
                      <th className="px-3 py-2 text-right">Discount</th>
                      <th className="px-3 py-2 text-right">Amount Paid</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.settledPurchases.map((item, idx) => (
                      <tr key={idx} className="border-b border-surface-3">
                        <td className="px-3 py-2 font-mono text-xs">{item.purchaseNo}</td>
                        <td className="px-3 py-2 text-right tabular-nums">{formatCurrency(item.purchaseAmount)}</td>
